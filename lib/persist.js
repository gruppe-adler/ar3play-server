...
exports.init = init;
function getIsPrivate(missionName, cb) {
    redisClient.hget(sprintf('mission:%s', missionName), 'is_private', function (err, data) {
        cb(err, data === '1');
    });
}
exports.getIsPrivate = getIsPrivate;
function setIsPrivate(isPrivate, cb) {
    getCurrentMission(function (err, missionName) {
        redisClient.hset('mission: ' + missionName, 'is_private', isPrivate ? '1' : '0', dummyCallback);
    });
    cb && cb(null, 201);
}
exports.setIsPrivate = setIsPrivate;
function setMissionName(name, cb) {
